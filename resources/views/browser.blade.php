...
<head>
    ...
    <style>
        ...
        * {
        ...
        /* Grid & Cards - SAMA PERSIS SEPERTI ASLINYA */
        .br-grid {
            display: grid;
        ...
        .br-toast.visible {
            ...
            animation: toastSlideInRight 0.4s ease forwards !important;
        }
        ...
        /* Responsive - SAMA PERSIS SEPERTI ASLINYA */
...
<body data-browser-detail-endpoint="{{ route('browser.get-detail') }}"
    ...
    <div class="br-page">
        ...
        <form method="GET" action="{{ route('browser.index') }}" id="filterForm">
            <div class="br-filter-bar">
                <!-- Row 1: Jurusan dan Program Studi sebelahan -->
                        ...
                        <div class="br-filter-item">
                            ...
                            <div style="display: flex; gap: 8px; align-items: center;">
                                <select class="br-select" name="filter_prodi" id="filterProdi"
                                    onchange="handleProdiChange(this.value)" style="flex: 1;"
                                    @disabled(!$filter_jurusan && !$filter_prodi)>
                                    <option value="">
                                    ...
                                    @if ($filter_jurusan || $filter_prodi)
                                        @foreach ($prodi_data as $prodi)
                                            <option value="{{ $prodi->id_prodi }}"
                                                data-jurusan="{{ $prodi->id_jurusan ?? '' }}"
                                                @selected((string) $filter_prodi === (string) $prodi->id_prodi)>
                                                {{ $prodi->nama_prodi }}
                                            </option>
                                        @endforeach
                                    @endif
            ...
            <!-- Row 2: Kontrol View dan Urutkan di kanan -->
            <div class="br-filter-row br-filter-tools-row">
                <div class="br-filter-tools">
                            ...
                            <div class="br-footer">
                                ...
                                <div class="br-actions">
                                    ...
                                    <a href="{{ $doc['download_url'] ?? '#' }}" download class="br-act br-act-dl"
                                        title="Unduh"
                                        onclick="event.stopPropagation();showToast('success','Mengunduh','File sedang diunduh...', 6000)">
    ...
    <script>
        ...
        function resetJurusanAndProdi(event) {
            ...
            showToast('success', 'Berhasil', 'Filter jurusan dan program studi telah direset', 3000);
        ...
        function loadProdiByJurusan(jurusanId) {
            ...
            fetch(url, {
                ...
                .then(data => {
                    ...
                    showToast('success', 'Berhasil', 'Data Program Studi diperbarui', 3000);
                })
                .catch(error => {
                    clearTimeout(timeoutId);
                    ...
                    prodiSelect.disabled = true;
                    showProdiLoading(false);
                    showErrorToast('Error', error.message || 'Gagal memuat data prodi', 5000);
                });
        }
        ...
        function showToast(type, title, message, duration = 6000) {
            ...
            toast.className = `br-toast ${type}`;
            toast.innerHTML = `
                <i class="bi ${icons[type] || icons.info} br-toast-icon"></i>
                <div class="br-toast-content">
                    <div class="br-toast-title">${escapeHtml(title || '')}</div>
                    <div class="br-toast-message">${escapeHtml(message || '')}</div>
                </div>
                <button class="br-toast-close" onclick="removeToast(this.parentElement)" aria-label="Close">
                    <i class="bi bi-x"></i>
                </button>
            `;
        ...
        function removeToast(toastEl) {
            ...
            toastEl.style.animation = 'toastSlideOutLeft 0.3s ease forwards';
            setTimeout(() => {
        ...
        function showDetail(docId) {
            if (!docId && docId !== 0) {
                showToast('error', 'Error', 'ID dokumen tidak valid', 5000);
                return;
            }
            _currentDocId = docId;
