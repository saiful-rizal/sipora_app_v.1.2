    ...
    <style>
        ...
        * {
        ...
        .br-search-btn i {
          ...
          /* ═══════════════════════════════════════════
              ✅ FILTER CARD - JURUSAN, PRODI, TEMA, JENIS
              ═══════════════════════════════════════════ */
        .br-filter-bar {
            background: var(--card);
            border-radius: var(--r-xl);
        ...
        .br-filter-row {
            ...
            grid-template-columns: minmax(180px, 1fr) minmax(220px, 1.1fr) minmax(180px, 1fr) minmax(180px, 1fr) auto auto !important;
            gap: 16px !important;
        ...
        /* ✅ Container untuk Dropdown Prodi */
        .br-prodi-wrap {
            display: flex !important;
            gap: 8px !important;
            align-items: center !important;
        ...
        .br-filter-item-prodi .br-select {
            width: 100% !important;
        }

        .br-filter-item-reset {
            align-self: flex-end;
            justify-self: end;
        }

        /* Tombol Reset di posisi paling kanan */
...
<body data-browser-detail-endpoint="{{ route('browser.get-detail') }}"
    ...
    <div class="br-page">
        ...
        <!-- ✅ CARD FILTER - Jurusan, Prodi, Tema, Jenis -->
        @php
            $normalized_filter_jenis = strtolower((string) ($filter_jenis ?? ''));
        @endphp
        <form method="GET" action="{{ route('browser.index') }}" id="filterForm">
            <div class="br-filter-bar">
                <div class="br-filter-row">
                    ...
                    <!-- Program Studi -->
                    <div class="br-filter-item br-filter-item-prodi">
                        <div class="br-filter-label">
                            <i class="bi bi-mortarboard-fill"></i>
                            <span>Program Studi</span>
                        </div>
                        <div class="br-prodi-wrap">
                    ...
                    <div class="br-filter-item">
                        ...
                        <select class="br-select" name="filter_tema" id="filterTema"
                            onchange="handleTemaChange(this.value)">
                            <option value="">Semua Tema</option>
                            @foreach ($tema_data as $tema)
                                <option value="{{ $tema->id_tema }}" @selected((string) $filter_tema === (string) $tema->id_tema)>
                                    {{ $tema->nama_tema }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="br-filter-item">
                        ...
                        <select class="br-select" name="filter_jenis" id="filterJenis"
                            onchange="handleJenisChange(this.value)">
                            <option value="">Semua Jenis</option>
                            <option value="pdf" @selected($normalized_filter_jenis === 'pdf')>PDF</option>
                            <option value="docx" @selected($normalized_filter_jenis === 'docx')>DOCX</option>
                        </select>
                    </div>

                    <!-- Reset Semua (jika ada filter aktif) -->
                    @if ($filter_jurusan || $filter_prodi || $filter_tema || $filter_jenis || request('q'))
                    ...
                    <div class="br-filter-item br-filter-item-reset">
                        <button type="button" class="btn-reset-jurusan-prodi" onclick="resetJurusanAndProdi(event)">
        ...
        @if ($filter_jurusan || $filter_prodi || $filter_tema || $filter_jenis || request('q'))
            <div class="br-active-filters">
                ...
                @if ($filter_tema)
                    @php
                        $tema_name = collect($tema_data)->firstWhere('id_tema', $filter_tema)->nama_tema ?? 'Tema';
                    @endphp
                    <div class="br-active-chip">
                        <i class="bi bi-tags-fill" style="font-size:11px;"></i>
                        <span>{{ $tema_name }}</span>
                        <a href="#" onclick="event.preventDefault();handleTemaChange('');">
                            <i class="bi bi-x-lg"></i>
                        </a>
                    </div>
                @endif
                @if ($filter_jenis)
                    @php
                        $jenis_name_map = ['pdf' => 'PDF', 'docx' => 'DOCX'];
                        $jenis_name = $jenis_name_map[$normalized_filter_jenis] ?? strtoupper((string) $filter_jenis);
                    @endphp
                    <div class="br-active-chip">
                        <i class="bi bi-collection-fill" style="font-size:11px;"></i>
                        <span>{{ $jenis_name }}</span>
                        <a href="#" onclick="event.preventDefault();handleJenisChange('');">
                            <i class="bi bi-x-lg"></i>
                        </a>
                    </div>
                @endif
                ...
                <div class="br-sort-group">
                    ...
                    <select class="br-sort-select" id="sortSelect" onchange="applySort(this.value)">
                        ...
                        @foreach($sortOptions as $sort)
                            <option value="?sort={{ $sort->sort_key }}" {{ (string) ($current_sort ?? request('sort', 'newest')) === (string) $sort->sort_key ? 'selected' : '' }}>
                                {{ $sort->sort_label }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="br-grid" id="documentGrid">
                @foreach ($documents as $index => $doc)
                    ...
                    <div class="br-card" data-id="{{ $doc['dokumen_id'] }}"
                        ...
                        data-tema="{{ $doc['id_tema'] ?? '' }}" data-jenis="{{ strtolower($doc['file_type'] ?? '') }}"
    ...
    <script>
        ...
        function resetJurusanAndProdi(event) {
            ...
            console.log('[RESET] Resetting all dropdown filters...');
            ...
            showToastCompact('success', 'Berhasil', 'Filter jurusan, program studi, tema, dan jenis dokumen telah direset', 3000);
        ...
        function handleJurusanChange(jurusanId) {
            ...
            const searchQuery = document.getElementById('searchInput')?.value || '';
            const temaId = document.getElementById('filterTema')?.value || '';
            const jenisId = document.getElementById('filterJenis')?.value || '';

            if (!jurusanId || jurusanId === '' || jurusanId === null) {
                resetProdiSelect();
                applyDocumentFilter('', '', temaId, jenisId, searchQuery);
                updateFilterQueryString('', '', temaId, jenisId);
                return;
            }

            applyDocumentFilter(jurusanId, '', temaId, jenisId, searchQuery);
            updateFilterQueryString(jurusanId, '', temaId, jenisId);
        ...
        function handleProdiChange(prodiId) {
            ...
            const jenisId = document.getElementById('filterJenis')?.value || '';
            const searchQuery = document.getElementById('searchInput')?.value || '';
            applyDocumentFilter(jurusanId, prodiId, temaId, jenisId, searchQuery);
            updateFilterQueryString(jurusanId, prodiId, temaId, jenisId);
        }

        function handleTemaChange(temaId) {
        ...
        function handleJenisChange(jenisId) {
            const jurusanId = document.getElementById('filterJurusan')?.value || '';
            const prodiId = document.getElementById('filterProdi')?.value || '';
            const temaId = document.getElementById('filterTema')?.value || '';
            const searchQuery = document.getElementById('searchInput')?.value || '';
        ...
        function handleSearchInput(query) {
            ...
            const temaId = document.getElementById('filterTema')?.value || '';
            const jenisId = document.getElementById('filterJenis')?.value || '';
            applyDocumentFilter(jurusanId, prodiId, temaId, jenisId, query);
            updateSearchQueryString(query);
        }

        function applyDocumentFilter(jurusanId, prodiId, temaId, jenisId, searchQuery = '') {
            ...
            const selectedTema = String(temaId || '').trim();
            const selectedJenis = String(jenisId || '').trim();
            const normalizedSearch = String(searchQuery || '').trim().toLowerCase();

            cards.forEach(card => {
                const cardJurusan = String(card.dataset.jurusan || '').trim();
                const cardProdi = String(card.dataset.prodi || '').trim();
                const cardTema = String(card.dataset.tema || '').trim();
                const cardJenis = String(card.dataset.jenis || '').trim();
                ...
                if (selectedTema && cardTema !== selectedTema) visible = false;
                if (selectedJenis && cardJenis !== selectedJenis) visible = false;
                if (normalizedSearch) {
        ...
        function updateFilterQueryString(jurusanId, prodiId, temaId, jenisId) {
            try {
                const url = new URL(window.location.href);
                if (jurusanId) {
                ...
                if (temaId) {
                    url.searchParams.set('filter_tema', temaId);
                } else {
                    url.searchParams.delete('filter_tema');
                }
                if (jenisId) {
                    url.searchParams.set('filter_jenis', jenisId);
                } else {
