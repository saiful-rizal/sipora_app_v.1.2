...
<head>
    ...
    <link
                                                @if (in_array((int) ($doc->status_id ?? 0), [1, 4], true))
                                                    <a href="{{ route('documents.edit', $doc->dokumen_id) }}"
                                                        class="btn-action btn-show" title="Edit Dokumen">
                                                        <i class="bi bi-pencil-square"></i>
                                                    </a>
                                                    <form action="{{ route('documents.destroy', $doc->dokumen_id) }}"
                                                        method="POST" onsubmit="return confirm('Hapus dokumen ini?')"
                                                        style="display:inline-block;">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit" class="btn-action btn-download"
                                                            title="Hapus Dokumen" style="border:none;">
