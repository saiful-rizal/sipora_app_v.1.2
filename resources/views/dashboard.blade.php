...
<head>
    ...
    <style>
        ...
        * {
        ...
        /* ═══ HERO - SESUAIKAN DENGAN BROWSE PAGE ═══ */
        .hero {
        ...
        .hero-left {
            ...
            justify-content: center;
            max-width: 100%;
            /* ✅ TAMBAHAN: Pastikan fleksibilitas penuh */
        }
        ...
        /* ✅ PARAGRAF - PERLEBAR DARI 340px JADI 480px (SESUAI BROWSE) */
        .hero p {
            font-size: 13.5px;
            font-family: var(--font-b);
            color: rgba(255, 255, 255, .75);
            max-width: 480px;
            /* ✅ UBAH: Dari 340px → 480px */
            line-height: 1.55;
            /* ✅ UBAH: Dari 1.5 → 1.55 (konsisten dengan browse) */
            margin-bottom: 0;
        }
        ...
        .modal-dialog {
            ...
            display: flex;
            flex-direction: column;
            box-shadow: 0 32px 80px rgba(11, 27, 77, .18);
...
<body data-detail-url="{{ route('dashboard.get-detail') }}" data-share-base="{{ url('/dashboard') }}">
                ...
                @foreach ($documents as $index => $doc)
                    @php
                        ...
                        if (str_contains($jenisDocRaw, 'tesis')) {
                            ...
                            str_contains($jenisDocRaw, 'tugas akhir') ||
                            str_contains($jenisDocRaw, 'tugas_akhir')
                        ) {
                    ...
                    <div class="doc-card" data-id="{{ $doc['dokumen_id'] }}" data-title="{{ strtolower($judul) }}"
                        data-description="{{ strtolower($abstrak) }}" data-ext="{{ $fileExt }}"
                        data-jenis-doc="{{ $jenisDoc }}" data-full-title="{{ $doc['judul'] ?? 'Tanpa Judul' }}"
                        ...
                        data-nama-tema="{{ $doc['nama_tema'] ?? '' }}" data-tahun="{{ $doc['tahun'] ?? '' }}"
                        data-status-id="{{ $statusId }}" data-status-name="{{ $statusName }}"
                        data-status-badge="{{ $badgeClass }}" data-turnitin="{{ $doc['turnitin'] ?? '' }}"
                        data-file-name="{{ $fileName }}" data-file-size="{{ $doc['file_size'] ?? 0 }}"
                        data-tgl-unggah="{{ $doc['tgl_unggah'] ?? '' }}"
                        data-updated-at="{{ $doc['tgl_unggah'] ?? '' }}"
                        data-id-user="{{ $doc['id_user'] ?? $doc['uploader_id'] ?? '' }}"
                        data-file-url="{{ $fileURL }}" data-file-type="{{ $fileExt }}"
                        ...
                        <div class="doc-body">
                            <div class="doc-header">
                                ...
                                <div class="doc-badges">
                                    ...
                                    @if ($jenisDoc !== 'lainnya')
                                        <span
                                            class="badge b-gray">{{ ucfirst(str_replace('_', ' ', $jenisDoc)) }}</span>
                                    @endif
                                </div>
                            </div>
                            ...
                            <div class="doc-meta">
                                @if (!empty($doc['nama_jurusan']))
                                    <span><i
                                            class="bi bi-briefcase"></i>{{ \Illuminate\Support\Str::limit($doc['nama_jurusan'], 18, '') }}</span>
                                @endif
                                @if (!empty($doc['tahun']))
                            ...
                            <div class="doc-footer">
                                ...
                                <div class="doc-actions">
                                    ...
                                    <!-- Tombol Preview dihapus sesuai permintaan -->
                                    <a href="{{ $fileURL }}" download class="act-btn act-dl" title="Unduh file"
    ...
    <script>
        ...
        function showDocumentPreview(docId, fileUrl, fileType) {
            ...
            if (fileType === 'pdf' && fileUrl) {
                ...
                container.innerHTML =
                    ...
                    (fileUrl ?
                        '<a href="' + fileUrl +
                        '" download class="download-link-min" style="margin-top:16px;">Unduh</a>' :
                        '') +
                    '</div>';
            }
        }
                ...
                fetch(detailUrl + '?id=' + docId)
                    ...
                    .then(function(data) {
                        if (data.success) infoContainer.innerHTML = buildInfoHTML(data.data || data);
                            ...
                            '<p style="color:var(--rose);text-align:center;padding:20px;">Gagal memuat detail.</p>';
                    })
                    .catch(function() {
                        infoContainer.innerHTML =
                            '<p style="color:var(--rose);text-align:center;padding:20px;">Terjadi kesalahan saat memuat data.</p>';
                    });
            }
        }
        ...
        function buildInfoHTML(d) {
            var rows = [{
                    label: 'Judul',
                    value: d.judul || d.full_title || '-'
                },
                {
                    label: 'Abstrak',
                    value: d.abstrak || d.full_description || '-'
                },
                {
                    label: 'Pengunggah',
                    value: (d.uploader_name || '-') + (d.uploader_email ? (' (' + d.uploader_email + ')') : '')
                }
            ];

            if (d.nama_jurusan) rows.push({
                label: 'Jurusan',
                value: d.nama_jurusan
            });
            if (d.nama_prodi) rows.push({
                label: 'Prodi',
                value: d.nama_prodi
            });
            if (d.nama_tema) rows.push({
                label: 'Tema',
                value: d.nama_tema
            });
            if (d.tahun) rows.push({
                label: 'Tahun',
                value: d.tahun
            });
            if (d.status_name) rows.push({
                label: 'Status',
                value: '<span class="info-badge-inline badge ' + (d.status_badge || 'b-gray') + '">' + d
                    .status_name + '</span>'
            });
            if (d.turnitin && Number(d.turnitin) > 0) rows.push({
                label: 'Turnitin',
                value: d.turnitin + '%'
            });
            ...
            rows.push({
                label: 'File',
                value: (d.file_name || '-') + ' (' + formatFileSize(d.file_size) + ')'
            });
            rows.push({
                label: 'Diunggah',
                value: formatDate(d.tgl_unggah || d.updated_at)
            });
            ...
            if (d.file_url) {
                html += '<div style="margin-top:20px;text-align:right;">' +
                    '<a href="' + d.file_url +
                    '" download class="download-link-min">' +
                    'Unduh</a></div>';
            }
