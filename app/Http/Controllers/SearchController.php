    ...
    public function index(Request $request): View|RedirectResponse
    {
        ...
        if ($searchQuery !== '') {
            ...
            switch ($sort) {
                ...
                case 'title_za':
                    ...
                    break;
                case 'newest':
                    $query->orderByDesc('d.tgl_unggah');
                    break;
                case 'relevance':
                default:
                    $query->orderByRaw('CASE WHEN LOWER(d.judul) LIKE ? THEN 0 ELSE 1 END', ['%' . strtolower($searchQuery) . '%'])
                        ->orderByDesc('d.tgl_unggah');
                    break;
            }

            $results = $query->get()->map(fn ($doc) => $this->mapDocument($doc));
        ...
        return view('search', [
            ...
            'sort_options' => DB::table('master_sort_options')
                ->whereIn('sort_scope', ['search', 'both'])
                ->orderBy('sort_order')
                ->get(),
            'current_sort' => $sort,
        ]);
