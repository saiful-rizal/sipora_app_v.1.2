...
class AdminMasterDataController extends Controller
{
    ...
    */
    ...
    public function jurusanIndex(Request $request): View|RedirectResponse
    {
        ...
        $jurusan = DB::table('master_jurusan as j')
            ...
            ->leftJoin('master_prodi as p', 'j.id_jurusan', '=', 'p.id_jurusan')
            ->select(
                'j.id_jurusan',
                'j.nama_jurusan',
                'j.id_rumpun',
                'r.nama_rumpun',
                DB::raw('COUNT(p.id_prodi) as total_prodi')
            )
            ->groupBy('j.id_jurusan', 'j.nama_jurusan', 'j.id_rumpun', 'r.nama_rumpun')
    ...
    */
    ...
    public function storeRumpun(Request $request): RedirectResponse
    {
        if ($redirect = $this->ensureAdmin($request)) {
            return $redirect;
        }
        ...
        $validated = $request->validate([
            'nama_rumpun' => ['required', 'string', 'max:100'],
        ]);

        DB::table('master_rumpun')->insert([
            'nama_rumpun' => trim($validated['nama_rumpun']),
        ]);

        return redirect()->route('admin.tema.index')
            ->with('success', 'Rumpun berhasil ditambahkan')
            ->with('active_tab', 'rumpun');
    }

    public function updateRumpun(Request $request, int $id): RedirectResponse
    {
        if ($redirect = $this->ensureAdmin($request)) {
            return $redirect;
        }
        ...
        return redirect()->route('admin.tema.index')
            ->with('success', 'Rumpun berhasil diperbarui')
            ->with('active_tab', 'rumpun');
    }

    public function deleteRumpun(Request $request, int $id): RedirectResponse
    {
        if ($redirect = $this->ensureAdmin($request)) {
            return $redirect;
        }
        ...
        if (!$this->isSuperAdmin($request->session()->get('auth_user')['role'] ?? null)) {
            return redirect()->route('admin.tema.index')
                ->with('error', 'Hanya Super Admin yang dapat menghapus rumpun')
                ->with('active_tab', 'rumpun');
        }
        ...
        return redirect()->route('admin.tema.index')
            ->with('success', 'Rumpun berhasil dihapus')
            ->with('active_tab', 'rumpun');
    }
    ...
    */
    ...
    public function usersReport(Request $request): View|RedirectResponse|BinaryFileResponse
    {
        ...
        $query = DB::table('users')
            ->select('id_user', 'nama_lengkap', 'username', 'nim', 'email', 'role', 'status', 'created_at')
            ->orderByDesc('created_at');
